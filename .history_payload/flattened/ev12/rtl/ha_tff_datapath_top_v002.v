...
module ha_tff_datapath_top_v002 (
    input  wire        clk,
    input  wire        rst,
    ...
    ha_tff_bram_bank #(.INIT_FILE("../../07_Simulations/sim005/bank0.mem")) bank0 (
        .clk(clk), .rst(rst),
        .read_addr(h0), .read_en(hash_valid),
        .read_data(b0_data), .read_valid(b0_v)
    );
    ha_tff_bram_bank #(.INIT_FILE("../../07_Simulations/sim005/bank1.mem")) bank1 (
        .clk(clk), .rst(rst),
        .read_addr(h1), .read_en(hash_valid),
        .read_data(b1_data), .read_valid(b1_v)
    );
    ha_tff_bram_bank #(.INIT_FILE("../../07_Simulations/sim005/bank2.mem")) bank2 (
        .clk(clk), .rst(rst),
        .read_addr(h2), .read_en(hash_valid),
        .read_data(b2_data), .read_valid(b2_v)
    );
    ha_tff_bram_bank #(.INIT_FILE("../../07_Simulations/sim005/bank3.mem")) bank3 (
        .clk(clk), .rst(rst),
