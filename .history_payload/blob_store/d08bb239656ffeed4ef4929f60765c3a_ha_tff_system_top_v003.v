...
// Hardware-Accelerated Traffic Filter Firewall (HA-TFF) - SYSTEM TOP v003
// Iteration 011: Fixes WNS (-0.465ns) timing violation and data payload misalignment.

module ha_tff_system_top_v003 (
    input  wire         clk,
    input  wire         rst,
    ...
    // PIPELINE STAGE: Break the combinational critical path!
    // dp_match_valid is asserted 4 cycles after the start of packet parsing.
    reg final_forward_reg;
    always @(posedge clk) begin
        if (rst) begin
            final_forward_reg <= 1'b0;
        end else begin
            // We combine the 4-cycle delayed datapath decision with the current SNN anomaly state
            // and register it to be used in cycle 5.
            final_forward_reg <= (dp_match_valid && dp_action_forward && !anomaly_detected);
        end
    end

    // DATA SYNCHRONIZATION: The decision requires 4 logic cycles + 1 pipeline register = 5 cycles total latency.
    // We delay the original AXI-Stream payload by exactly 5 cycles so they align perfectly.
    ...
    // Only forward the delayed valid signal if our cycle 5 registered decision allows it.
